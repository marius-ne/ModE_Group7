"""Solve the LP approximation on a single price ratio and save the dispatch.

solve_and_save() is the reusable part: plot_lp_approx_dispatch.py imports it to solve on
demand rather than requiring this script to be run first. There is no delta plot for this
formulation: the LP approximation has no commitment variables.

    python Marius/evaluation/run_lp_approx_dispatch.py [ratio]
"""
...
DEFAULT_RATIO = 0.5
C_EL = 1.0
MODE = "mean_efficiency"  # how the dropped commitment is approximated; see solve_lp_approximated
...
def solve_and_save(ratio: float) -> tuple[pd.DataFrame, dict]:
    """Solve the LP approximation at this price ratio, save the dispatch, and return it
    (dispatch, meta)."""
    c_g = C_EL * ratio

    print(f"LP approx  c_el={C_EL}  c_G={c_g}  ratio={ratio}  mode={MODE}")
    opex, dispatch = solve_lp_approximated(Q_D, P_D, c_g, C_EL, mode=MODE,
