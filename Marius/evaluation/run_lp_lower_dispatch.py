"""Solve the LP lower bound on a single price ratio and save the dispatch.

solve_and_save() is the reusable part: plot_lp_lower_dispatch.py imports it to solve on demand
rather than requiring this script to be run first.

    python Marius/evaluation/run_lp_lower_dispatch.py [ratio]
"""
...
def solve_and_save(ratio: float) -> tuple[pd.DataFrame, dict]:
    """Solve the LP lower bound at this price ratio, save the dispatch, and return it
    (dispatch, meta)."""
    c_g = C_EL * ratio

    print(f"LP lower  c_el={C_EL}  c_G={c_g}  ratio={ratio}")
    opex, dispatch = solve_lp_lower(Q_D, P_D, c_g, C_EL,
