...
Erdem's own binary on/off panel, same as the MILP's. The title says which heuristic produced
the schedule. Solves the given ratio itself (via
...
def main():
    ...
    mode = meta["mode"]
    name = f"LP upper bound ({MODE_LABELS.get(mode, mode)})"
