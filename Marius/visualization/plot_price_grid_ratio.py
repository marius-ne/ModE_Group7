...
def plot_price_grid_ratio(output_dir: str | None = None, fontsize: int = 11) -> Path:
    ...
    fig, ((ax_grid, ax_hist), (ax_empty, ax_hist_log)) = plt.subplots(2, 2, figsize=(12, 9))
    ax_empty.axis("off")
    ...
    ax_grid.scatter(c_el_grid.ravel(), c_G_grid.ravel(), color=_BLUE, s=25,
    ...
    ax_grid.set_xlabel(r"Electricity price $c_{\mathrm{el}}$ [€/MWh]", fontsize=fontsize)
    ax_grid.set_ylabel(r"Gas price $c_G$ [€/MWh]", fontsize=fontsize)
    ax_grid.set_title(f"Feasible price rectangle — {N_GRID}×{N_GRID} grid", fontsize=fontsize, fontweight="bold")
    ax_grid.tick_params(labelsize=fontsize - 1)
    ax_grid.grid(True, linewidth=0.4, alpha=0.5)
    ...
    ax_hist.set_xlabel(r"Price ratio $c_G\,/\,c_{\mathrm{el}}$ [-]", fontsize=fontsize)
    ax_hist.set_ylabel("Probability density", fontsize=fontsize)
    ax_hist.set_title(f"Price ratios of the {len(ratios)} grid samples", fontsize=fontsize, fontweight="bold")
    ...
    ax_hist_log.set_xlabel(r"Price ratio $c_G\,/\,c_{\mathrm{el}}$ [-]", fontsize=fontsize)
    ax_hist_log.set_ylabel("Probability density", fontsize=fontsize)
    ax_hist_log.set_title(f"Price ratios of the {len(ratios)} grid samples — log scale", fontsize=fontsize, fontweight="bold")
    ax_hist_log.tick_params(labelsize=fontsize - 1)
    ax_hist_log.grid(True, which="both", linewidth=0.4, alpha=0.5)
